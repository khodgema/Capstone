...
def draw_court(ax=None, color='gray', lw=1, outer_lines=False, zorder=2):
    ...
    # Create court elements with zorder
    hoop = Circle((0, 0), radius=7.5, linewidth=lw, color=color, fill=False, zorder=zorder)
    backboard = Rectangle((-30, -12.5), 60, 0, linewidth=lw, color=color, zorder=zorder)
    outer_box = Rectangle((-80, -47.5), 160, 190, linewidth=lw, color=color, fill=False, zorder=zorder)
    inner_box = Rectangle((-60, -47.5), 120, 190, linewidth=lw, color=color, fill=False, zorder=zorder)
    top_free_throw = Arc((0, 142.5), 120, 120, theta1=0, theta2=180, linewidth=lw, color=color, fill=False, zorder=zorder)
    bottom_free_throw = Arc((0, 142.5), 120, 120, theta1=180, theta2=0, linewidth=lw, color=color, linestyle='dashed', zorder=zorder)
    restricted = Arc((0, 0), 80, 80, theta1=0, theta2=180, linewidth=lw, color=color, zorder=zorder)
    corner_three_a = Rectangle((-220, -47.5), 0, 140, linewidth=lw, color=color, zorder=zorder)
    corner_three_b = Rectangle((220, -47.5), 0, 140, linewidth=lw, color=color, zorder=zorder)
    three_arc = Arc((0, 0), 475, 475, theta1=22, theta2=158, linewidth=lw, color=color, zorder=zorder)
    center_outer_arc = Arc((0, 422.5), 120, 120, theta1=180, theta2=0, linewidth=lw, color=color, zorder=zorder)
    center_inner_arc = Arc((0, 422.5), 40, 40, theta1=180, theta2=0, linewidth=lw, color=color, zorder=zorder)
    ...
    if outer_lines:
        outer_lines = Rectangle((-250, -47.5), 500, 470, linewidth=lw, color=color, fill=False, zorder=zorder)
...
def shot_chart(x, y, kind="scatter", title="", color="b", cmap=None,
               ...
               despine=False, zorder_kde=1, **kwargs):
    """
    ...
    # Draw court with lower zorder
    draw_court(ax, color=court_color, lw=court_lw, outer_lines=outer_lines, zorder=2)
    
    if kind == "scatter":
        ax.scatter(x, y, c=color, zorder=zorder_kde, **kwargs)
    
    elif kind == "kde":
        # KDE plot with higher zorder
        sns.kdeplot(x=x, y=y, fill=kde_shade, cmap=cmap, ax=ax, zorder=zorder_kde, **kwargs)
        ax.set_xlabel('')
        ax.set_ylabel('')
        ...
        ax.hexbin(x, y, gridsize=gridsize, cmap=cmap, zorder=zorder_kde, **kwargs)
